...
# Checking for errors
for e in $VARNISH_ERROR $PHPFPM_ERROR $HHVM_ERROR $APACHE_ERROR $NGINX_ERROR $MYSQL_ERROR $MONGODB_ERROR $CASSANDRA_ERROR $COUCHDB_ERROR $SUBVERSION_ERROR $SOLR_ERROR $ELASTICSEARCH_ERROR $TOMCAT_ERROR $REDIS_ERROR $RESIN_ERROR $MEMCACHED_ERROR $INGRES_ERROR $LIBREOFFICE_ERROR $LUCENE_ERROR $ZOPE_ERROR $POSTGRESQL_ERROR $THIRD_ERROR $NAGIOS_ERROR $RABBITMQ_ERROR $ACTIVEMQ_ERROR $ZOOKEEPER_ERROR $KAFKA_ERROR $JETTY_ERROR $JBOSS_ERROR $WILDFLY_ERROR $HYPERSONIC_ERROR $ELASTICSEARCH_ERROR $LOGSTASH_ERROR $LOGSTASHFORWARDER_ERROR; do
    if [ $e -gt 0 ]; then
        ERROR=$e
    fi
done
